...
The term was coined in [the original local-first essay](https://www.inkandswitch.com/essay/local-first/)
(or its [pdf version](https://martin.kleppmann.com/papers/local-first.pdf)) and if you haven't read that, you should. This essay isn't going to make much sense if you haven't.

I would also recommend Martin Kleppmann's
[much-simplified definition](https://martin.kleppmann.com/2024/05/30/local-first-conference.html), based on an inversion of Leslie Lamport's definition of a distributed system:
...
In this scenario, a "service" isn't a special piece of software. It's just a regular p2p node like anyone might run on their phone or laptop. But it's guaranteed to stay up 24/7 and it might be beefier, geographically distributed to reduce latency, or any of the umpteen other reasons we created The All-Powerful Cloud.

On the flip side, it's worth noting that even though the local-first scene is rife with p2p tooling, "local-first" doesn't make p2p a _requirement._ Local-first apps can sync through a non-peer remote service. But client/server architectures need some way for users to run their own servers if p2p isn't an option. Which, as we'll see, heavily implies that those services must be open source.
...
I put [Linear](https://linear.app/) in this category. When (not if) Linear Orbit, Inc. finally dies, you can't keep your data. You're not going to manage your software project with a CSV export and their software will be gone forever. I have other problems with Linear, but I'll get to those later.
...
Sebastian Wick and I had
[a back-and-forth thread about this](https://fantastic.earth/@deobald/115313381526877943)
a few months ago. While "multi-device"
...
* magic: NAT holepunching and service discovery, in the case of p2p nodes
